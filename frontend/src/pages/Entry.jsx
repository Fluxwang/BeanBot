...
function accountLabel(name) { return name.split(':').pop(); }
...
function LetterIcon({ name, size = 38 }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontSize: size * 0.38, fontWeight: 700, color }}>{label[0]?.toUpperCase()}</span>
    </div>
  );
}
...
function NumPad({ onKey, accent }) {
  ...
  return (
    <div style={{ padding: 12, display: 'grid', gap: 8, gridTemplateColumns: 'repeat(4, 1fr)', gridAutoRows: 56, background: '#0a0a0a' }}>
      {keys.flat().map((k, i) => {
        ...
        return (
          <button key={i} onClick={() => onKey(k.id)} style={{
            ...
            fontSize: k.confirm ? 15 : 22, fontWeight: k.confirm ? 600 : 500,
            lineHeight: 1,
          }}>
            {k.label}
...
function AccountSheet({ title, accounts, currentId, excludeId, onPick, onClose }) {
  return (
          ...
          {accounts.map((a, i) => {
            ...
            return (
              <button key={a.name} onClick={() => !disabled && onPick(a.name)} style={{
                width: '100%', border: 0, background: 'transparent', padding: '12px 4px', cursor: disabled ? 'default' : 'pointer',
                display: 'flex', alignItems: 'center', gap: 14, opacity: disabled ? 0.35 : 1,
...
function groupAccounts(accounts) {
  ...
  for (const a of accounts) {
    ...
    if (!groups[groupKey]) groups[groupKey] = { key: groupKey, label: groupKey, children: [] };
...
function CategoryGrid({ accounts, value, onChange }) {
  ...
  // 当前选中的 group
  const selectedGroup = value ? groups.find(g => g.children.some(c => c.name === value)) : null;

  return (
    <>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '14px 8px', padding: '4px 4px 12px' }}>
        {visible.map(g => {
          ...
          return (
            <button key={g.key} onClick={() => {
              ...
              display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, padding: '4px 0', fontFamily: 'inherit',
            }}>
              <div style={{
                width: 48, height: 48, borderRadius: '50%',
                background: active ? color : color + '24',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                transition: 'background 0.15s', boxShadow: active ? `0 4px 16px ${color}55` : 'none',
              }}>
                <span style={{ fontSize: 20, fontWeight: 700, color: active ? '#0a0a0a' : color }}>
                  {g.label[0]?.toUpperCase()}
                </span>
              </div>
              <span style={{ fontSize: 11, color: active ? '#f5f5f5' : 'rgba(255,255,255,0.65)', fontWeight: active ? 600 : 500, textAlign: 'center', lineHeight: 1.2 }}>
                {g.label}
              </span>
            </button>
          );
...
// ── MetaRow ──────────────────────────────────────────────────
function MetaRow({ assetId, assets, note, payee, onNoteChange, onPickAsset, onPickPayee }) {
  const chip = (icon, text, onClick, hint) => (
    <button onClick={onClick} style={{
      border: 0, background: '#1e1e1e', color: '#dcdcdc',
      ...
      <span>{text}</span>
      {hint && <span style={{ color: 'rgba(255,255,255,0.35)', fontSize: 11 }}>{hint}</span>}
    </button>
  );
    ...
    <div style={{ padding: '0 16px 10px' }}>
      <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 2 }}>
        ...
        <button onClick={onPickPayee} style={{
          border: 0, background: '#1e1e1e', color: payee ? '#f5f5f5' : 'rgba(255,255,255,0.4)',
          borderRadius: 999, padding: '7px 12px', fontFamily: 'inherit', fontSize: 13, cursor: 'pointer', flexShrink: 0,
        }}>
          {payee || '商家（选填）'}
        </button>
      </div>
...
function PayeeSheet({ value, onChange, onClose }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 30, display: 'flex', flexDirection: 'column' }}>
      <div onClick={onClose} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.55)' }} />
      <div style={{ marginTop: 'auto', position: 'relative', background: '#1a1a1a', borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: '16px 20px 40px' }}>
        <div style={{ color: '#f5f5f5', fontSize: 15, fontWeight: 600, marginBottom: 14 }}>商家名称（选填）</div>
        <div style={{ display: 'flex', gap: 10 }}>
          <input autoFocus value={input} onChange={e => setInput(e.target.value)}
            placeholder="例：KFC、星巴克..."
            style={{ flex: 1, height: 46, padding: '0 14px', background: '#2a2a2a', border: 0, borderRadius: 12, color: '#f5f5f5', fontSize: 15, fontFamily: 'inherit', outline: 'none' }} />
          <button onClick={() => { onChange(input); onClose(); }} style={{
            height: 46, padding: '0 18px', background: '#c8a96e', border: 0, borderRadius: 12,
            color: '#000', fontSize: 15, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit',
          }}>确认</button>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{ flex: 1, padding: '0 16px', display: 'flex', flexDirection: 'column', minHeight: 0, overflowY: 'auto' }}>
      ...
      <div style={{ background: '#1a1a1a', borderRadius: 16, padding: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
        <textarea value={text} onChange={e => { setText(e.target.value); setParsed(null); }}
          placeholder={'例如:  35 CMB KFC 午饭\n     咖啡 22 manner 支付宝\n     打车 48.5 滴滴'}
          style={{ width: '100%', minHeight: 90, resize: 'none', background: 'transparent', border: 0, outline: 'none', color: '#f5f5f5', fontFamily: 'inherit', fontSize: 16, lineHeight: 1.5, padding: 0 }} />
      ...
      <button onClick={handleParse} disabled={!text.trim() || parsing} style={{
        ...
        cursor: text.trim() ? 'pointer' : 'default', fontFamily: 'inherit', fontSize: 15, fontWeight: 600,
      }}>
        {parsing ? '解析中...' : '✦ 解析'}
      </button>
      ...
      {parsed && (
        <div style={{ marginTop: 12, background: '#1a1a1a', borderRadius: 16, border: `1px solid ${accent}55`, padding: '14px 14px 12px' }}>
          ...
          <div style={{ fontSize: 30, fontWeight: 700, color: '#f5f5f5', letterSpacing: -1, marginBottom: 8 }}>
            <span style={{ fontSize: 18, color: '#e9876a' }}>−¥</span>{parsed.amount?.toFixed(2)}
          </div>
          {[
          ...
          <button onClick={handleConfirm} disabled={submitting} style={{
            marginTop: 10, width: '100%', height: 40, border: 0, background: accent,
            color: '#0a0a0a', borderRadius: 10, cursor: 'pointer', fontFamily: 'inherit', fontSize: 14, fontWeight: 600,
          }}>
            {'确认入账 →'}
          </button>
        </div>
...
export default function Entry() {
  ...
  return (
    <div style={{ height: '100dvh', display: 'flex', flexDirection: 'column', background: '#0a0a0a', color: '#f5f5f5', fontFamily: '-apple-system,"SF Pro",system-ui,sans-serif', position: 'relative', overflow: 'hidden' }}>
          ...
          {[{ id: 'exp', label: '支出' }, { id: 'inc', label: '收入' }, { id: 'tr', label: '转账' }].map(o => {
            ...
            return (
              <button key={o.id} onClick={() => { setKind(o.id); setCatId(null); }} style={{
                height: 30, padding: '0 14px', border: 0, cursor: 'pointer', borderRadius: 999, fontFamily: 'inherit',
      ...
      {mode === 'text' ? (
        ...
        <>
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            <TransferBody fromId={fromId} toId={toId} assets={accounts.assets}
              onPickFrom={() => setSheet('from')} onPickTo={() => setSheet('to')}
        ...
        <>
          ...
          <div style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
            ...
            <div style={{ height: 10 }} />
            <MetaRow assetId={assetId} assets={accounts.assets} note={note} payee={payee}
              onNoteChange={setNote} onPickAsset={() => setSheet('asset')}
      ...
      {/* 账户选择弹层 */}
      {sheet === 'asset' && (
        <AccountSheet title="选择账户" accounts={accounts.assets} currentId={assetId}
          onPick={(id) => { setAssetId(id); setSheet(null); }} onClose={() => setSheet(null)} />
      )}
      {sheet === 'from' && (
        <AccountSheet title="转出账户" accounts={accounts.assets} currentId={fromId} excludeId={toId}
          onPick={(id) => { setFromId(id); setSheet(null); }} onClose={() => setSheet(null)} />
      )}
      {sheet === 'to' && (
        <AccountSheet title="转入账户" accounts={accounts.assets} currentId={toId} excludeId={fromId}
          onPick={(id) => { setToId(id); setSheet(null); }} onClose={() => setSheet(null)} />
      )}
      {sheet === 'payee' && (
        <PayeeSheet value={payee} onChange={setPayee} onClose={() => setSheet(null)} />
      )}
    </div>
  );
}
